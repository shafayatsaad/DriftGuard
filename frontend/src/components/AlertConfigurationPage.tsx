...
const AlertConfigurationPage: React.FC = () => {
  ...
  const handleSaveRule = () => {
    if (editingId) {
      ...
      const rule: AlertRule = {
        ...
        severity: newRule.severity as any,
        type: newRule.type,
        condition: `${newRule.metric} ${newRule.operator} ${newRule.threshold || '0.90'}`,
        currentValue: "Pending...",
        isTriggered: false,
        channels: Object.keys(newRule.channels).filter(k => (newRule.channels as any)[k]),
        lastTriggered: "Never"
      };
      setRules([rule, ...rules]);
    }
    ...
    setNewRule({
      ...
      window: '1h',
      severity: 'critical',
      channels: { email: true, slack: false, pagerduty: false, webhook: false, sms: false },
      frequency: 'immediate'
    });
  };
  ...
  const handleEdit = (rule: AlertRule) => {
    ...
    // Simplified parsing of condition for demo purposes
    const conditionParts = rule.condition.split(' ');
    const metric = conditionParts[0] || 'Accuracy';
    const operator = conditionParts[1] || '<';
    const threshold = conditionParts[2]?.replace('%', '') || '0.90';

    setNewRule({
      ...newRule,
      name: rule.name,
      description: rule.note || '', // Assuming note can be description
      type: rule.type,
          ...
          {isCreating ? (
            ...
            <div className="bg-[#16213e] rounded-xl border border-gray-800 p-8 shadow-lg animate-slideInRight">
              <h2 className="text-xl font-bold text-white mb-6 flex items-center border-b border-gray-700 pb-4">
                <Plus className="w-5 h-5 mr-2 text-[#0f4c75]" /> {editingId ? 'Edit Alert Rule' : 'Create New Alert Rule'}
              </h2>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        ...
                        <select 
                          ...
                          className="w-full bg-[#16213e] border border-gray-600 rounded p-2 text-sm text-white"
                        >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <select 
                          ...
                          onChange={e => setNewRule({...newRule, window: e.target.value})}
                          className="w-full bg-[#16213e] border border-gray-600 rounded p-2 text-sm text-white"
                        >
                      ...
                      {['critical', 'warning', 'info'].map((sev) => (
                        <label key={sev} className="flex items-center cursor-pointer">
                          <input 
                            ...
                            checked={newRule.severity === sev}
                            onChange={() => setNewRule({...newRule, severity: sev})}
                          />
                     ...
                     <div className="space-y-3 bg-[#1a1a2e] p-4 rounded-lg border border-gray-700">
                        <label className="flex items-start space-x-3 cursor-pointer">
                           <input 
                            ...
                            onChange={e => setNewRule({...newRule, channels: {...newRule.channels, email: e.target.checked}})}
                           />
                        ...
                        <label className="flex items-start space-x-3 cursor-pointer">
                           <input 
                            type="checkbox" 
                            className="mt-1" 
                            checked={newRule.channels.slack}
                            onChange={e => setNewRule({...newRule, channels: {...newRule.channels, slack: e.target.checked}})}
                           />
                           <div className="flex-1">
                             <div className="flex items-center text-sm font-medium text-gray-200">
                        ...
                        <label className="flex items-start space-x-3 cursor-pointer">
                           <input 
                            type="checkbox" 
                            className="mt-1" 
                            checked={newRule.channels.pagerduty}
                            onChange={e => setNewRule({...newRule, channels: {...newRule.channels, pagerduty: e.target.checked}})}
                           />
                        ...
                        <label className="flex items-start space-x-3 cursor-pointer">
                           <input 
                            ...
                            onChange={e => setNewRule({...newRule, channels: {...newRule.channels, webhook: e.target.checked}})}
                           />
                           <div className="flex items-center text-sm font-medium text-gray-400">
              ...
              <div className="mt-8 flex justify-end space-x-4 pt-6 border-t border-gray-700">
                ...
                >
                  <Check className="w-4 h-4 mr-2" /> {editingId ? 'Save Changes' : 'Create Alert Rule'}
                </button>
              </div>
                ...
                <div key={rule.id} className="bg-[#16213e] rounded-xl border border-gray-800 p-6 shadow-md hover:border-gray-600 transition-colors">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex items-center space-x-3">
                       ...
                       <button onClick={() => handleEdit(rule)} className="text-gray-500 hover:text-white p-1"><Edit2 className="w-4 h-4" /></button>
                       <button onClick={() => handleDelete(rule.id)} className="text-gray-500 hover:text-[#e74c3c] p-1"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </div>
