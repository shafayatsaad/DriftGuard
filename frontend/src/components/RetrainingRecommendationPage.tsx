...
const RetrainingRecommendationPage: React.FC<RetrainingPageProps> = ({ onBack }) => {
  const handleSchedule = () => {
    ...
    window.alert("Retraining job scheduled for tonight at 02:00 AM UTC.");
  };

  const handleReport = () => {
    // Mock report download
    const reportContent = "RETRAINING RECOMMENDATION REPORT\n\nStatus: Urgent\nReason: Drift Detected\nRecommended Action: Retrain immediately.\n\nData Snapshot:\n- Accuracy: 87.3%\n- Drift Score: 0.31\n\nGenerated by DriftGuard.";
    const blob = new Blob([reportContent], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.setAttribute('download', 'retraining_recommendation.txt');
  ...
  const handleDismiss = () => {
    if (window.confirm("Are you sure you want to dismiss this recommendation? The model risk will remain critical.")) {
      onBack();
    }
  };
             ...
             <div className="flex flex-col space-y-3">
               ...
               <div className="grid grid-cols-2 gap-3">
                 <button 
                  ...
                  className="py-3 bg-[#0f4c75] hover:bg-[#165a8a] text-white font-medium rounded-lg flex items-center justify-center transition-colors"
                 >
                    <Calendar className="w-4 h-4 mr-2" /> Schedule
                 </button>
                 <button 
                  onClick={handleReport}
                  className="py-3 border border-gray-600 hover:border-gray-400 text-gray-300 font-medium rounded-lg flex items-center justify-center transition-colors"
                 >
               ...
               <button 
                ...
                className="w-full py-2 text-gray-500 hover:text-gray-300 text-sm flex items-center justify-center mt-2"
               >
